...
export const Start: t_signatures.Start = ($,) => _p.decide.state($, ($,): t_out.Start => {
    switch ($[0]) {
        case 'absolute':
            return _p.ss($, ($,) => ['absolute', null])
        case 'relative':
            return _p.ss($, ($,) => ['relative', ({
...
export const Non_Normalized_Path: t_signatures.Non_Normalized_Path = ($,) => ({
    ...
    'segments': _p.deprecated_cc($['segments'], ($,) => $.__l_map(($,) => _p.decide.state($, ($,): t_out.Non_Normalized_Path.segments.L => {
        switch ($[0]) {
            case 'parent':
                return _p.ss($, ($,) => ['parent', null])
            case 'child':
                return _p.ss($, ($,) => ['child', $])
            case 'current':
