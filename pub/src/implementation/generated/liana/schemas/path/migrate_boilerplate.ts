...
export const Start: t_signatures.Start = ($) => _p.deprecated_cc($, ($): t_out.Start => {
    switch ($[0]) {
        case 'absolute': return _p.ss($, ($) => ['absolute', null])
        case 'relative': return _p.ss($, ($) => ['relative', ({
...
export const Node_Path: t_signatures.Node_Path = ($) => ({
    ...
    'node': _p.deprecated_cc($['node'], ($) => $),
})
export const Non_Normalized_Path: t_signatures.Non_Normalized_Path = ($) => ({
    'leading slash': _p.deprecated_cc($['leading slash'], ($) => $),
    'segments': _p.deprecated_cc($['segments'], ($) => $.__l_map(($) => _p.deprecated_cc($, ($): t_out.Non_Normalized_Path.segments.L => {
        switch ($[0]) {
            case 'parent': return _p.ss($, ($) => ['parent', null])
            case 'child': return _p.ss($, ($) => ['child', $])
            case 'current': return _p.ss($, ($) => ['current', null])
            case 'nothing': return _p.ss($, ($) => ['nothing', null])
            default: return _p.au($[0])
        }
    }))),
    'trailing slash': _p.deprecated_cc($['trailing slash'], ($) => $),
})
